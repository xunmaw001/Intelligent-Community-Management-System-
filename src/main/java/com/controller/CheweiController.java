...
//                            cheweiEntity.setCheweiUuidNumber(data.get(0));                    //社区车位编号 要改的
//                            cheweiEntity.setCheweiPhoto("");//详情和图片
//                            cheweiEntity.setCheweiTypes(Integer.valueOf(data.get(0)));   //社区车位类型 要改的
...
//                            cheweiEntity.setCreateTime(date);//时间
                        ...
                        if(cheweiEntities_cheweiUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [社区车位编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
